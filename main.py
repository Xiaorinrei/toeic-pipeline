...
# ------------------------------------------------------------
# ネタ(トピック)と切り口。topics.txt があればそれを優先して読む
# ------------------------------------------------------------
# デフォルトのネタ一覧(topics.txt が無い場合に使う)
DEFAULT_TOPICS = [
    "英語の構造化(SVOC・品詞の理解)と瞬発力の鍛え方",
    "英語が瞬時に出てこない原因と、頭の中の翻訳をやめる方法",
    "リスニングが伸びない人がやりがちな勉強法と改善策",
    "ビジネスメールで使える英語表現と失礼にならない言い回し",
    "TOEICスコアと実際の英会話力のギャップを埋める方法",
    "英単語を「使える語彙」に変えるアウトプット練習",
    "オンライン会議で発言できるようになる英語の型",
    "英語の発音より大事な「伝わる話し方」のコツ",
    "スキマ時間でできる社会人向け英語学習ルーティン",
    "英語の音読・シャドーイングの正しいやり方と効果",
]

# 切り口(構成)をランダムに変えて、同じネタでも印象を変える
ANGLES = [
    "よくある失敗例を挙げて、その解決策を示す形式",
    "ステップ・バイ・ステップの実践ガイド形式",
    "『やりがちな勘違い』を正す、ミス指摘形式",
    "ビフォー・アフターの体験談風の形式",
    "チェックリスト付きの実用ノート形式",
    "Q&A形式で読者の疑問に答える形式",
...
def load_topics():
    path = os.environ.get("TOPICS_FILE", "topics.txt")
...
def generate_article():
    ...
    今回のテーマ:「{topic}」
    今回の切り口:「{angle}」
    このテーマと切り口で、noteにそのまま公開できる完成度の日本語ブログ記事を作成してください。
